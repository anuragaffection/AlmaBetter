...

{
    ...
    // this solution is wrong, we have to find the word in reqular ways
    // we do not have to find, each letters of word at any place, 

    // this solution is not passing all the test case
        ...
        for (let i = 0; i < board.length; i++) {
            ...
            for (let j = 0; j < board[i].length; j++) {
                ...
                if (word.includes(board[i][j])) {
                    ...
                    let index = word.indexOf(board[i][j]);

                    let first = word.slice(0, index);
                    let last = word.slice(index + 1, word.length);
